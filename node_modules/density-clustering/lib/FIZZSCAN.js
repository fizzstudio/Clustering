...
FIZZSCAN.prototype.run = function(dataset, epsilon, minPts, forceIn, distanceFunction) {
  ...
  if (this.forceIn){
    ...
    for (let noisePointID of this.noise){
      let nearestNeighbors = this._nNIndices(this.dataset, noisePointID);
      let i = 0;
      while (!(reverseClusterLookup[nearestNeighbors[i]])){
        i++;
      }
      tempStorage.push([noisePointID, reverseClusterLookup[nearestNeighbors[i]]])
    }
...
FIZZSCAN.prototype._nNIndices = function(dataset, pointId) {
  ...
  for (var id = 0; id < dataset.length; id++) {
    var dist = [id, this._euclideanDistance(dataset[pointId], dataset[id])];
    distances.push(dist);
  }
